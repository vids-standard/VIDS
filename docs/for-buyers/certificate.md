...
VIDS Validation Certificates are issued by approved evaluation operators following successful validation using the VIDS Reference Validator. Princeton Medical Systems is the current sole operator.

**Self-issued certificates, certificates issued by parties other than approved operators, or certificates modified after issuance do not constitute VIDS validation.** Buyers receiving a certificate from an unrecognized issuer should request the underlying `validation_report.json` and re-run the Reference Validator independently to verify the result.
...
<h4 style="text-align: center; color: #1F3864; font-weight: normal; margin-top: -1em;">Validation Certificate</h4>
...
<p>This certificate confirms that the dataset structure, metadata, annotation provenance, and quality documentation meet all requirements defined under the VIDS Full profile at the time of validation.</p>

<p><strong>Conditions</strong></p>
<ul>
<li>This validation applies only to the dataset version specified above.</li>
<li>Any modification to dataset contents or structure invalidates this certificate.</li>
<li>This certificate does not assess the clinical correctness of annotations.</li>
...
A VIDS Validation Certificate carries seven identifying fields:

- **Dataset name and version** — identifies the exact dataset to which the certificate applies. A new version requires a new certificate.
...
- **Validation Artifact** — the `validation_report.json` file that supports the certificate. Always attached or referenced.

## How buyers use the certificate

When a vendor delivers a dataset under a contract that includes the [VIDS Reference Procurement Language](sow-addendum.md), the certificate accompanies the delivery as evidence of compliance. The buyer's acceptance process is:

1. Receive the dataset, the certificate, and the supporting `validation_report.json`.
2. Independently re-run the VIDS Reference Validator on the delivered dataset.
3. Confirm that the buyer's re-validation produces the same result reported on the certificate.
...
The certificate is only as authoritative as the underlying validator output. Buyers should always verify rather than trust on face value.

## When a certificate is invalidated

The certificate ceases to apply if any of the following occurs:
...
In each case, a new validation must be performed and a new certificate issued for the modified dataset version.
